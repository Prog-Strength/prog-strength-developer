...
The dispatch pipeline guarantees **at most one active worker per SOW**. Before launching anything, the "Dispatch SOW" workflow acquires a lock on the SOW in a DynamoDB **run registry** (`prog-strength-developer-runs`). If a worker is already building that SOW, the dispatch is refused and **no instance is launched**. That stops an accidental double-dispatch (a mis-click, an over-eager re-run of a workflow thought to have failed) from fanning out duplicate workers that race each other into conflicting PRs — and from burning the EC2 + Claude cost of doing so. The worker releases the lock when it finishes; a stale lock (a worker that died without releasing) self-heals via the row's `expires_at` TTL.

The table holds two item types per ticket, keyed by a sort key. The **lock item** (`sk = "LOCK"`) is the mutable, TTL-eligible row the conditional write above operates on. Alongside it, each dispatch appends an immutable **run-history item** (`sk = "RUN#<started_at>#<dispatch_id>"`) — a permanent record of that autonomous-developer session (status, outcome, duration, PRs opened, `doc_type`, `compute_type`). Run items carry no `expires_at`, so the TTL never reaps them; a re-dispatch appends a new row rather than clobbering the prior one. This makes the table both the lock and a durable session log that periodic scans can aggregate into metrics (run counts, durations, failure/retry rates, per-doc-type breakdowns). The scanner/metrics emitter itself is a planned follow-on; see `docs/superpowers/specs/2026-06-16-run-history-table-design.md`.
