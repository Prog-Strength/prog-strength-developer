...
def _registry_with_runs():
    ...
    reg.attach_instance("sows/a.md", dispatch_id="d1", instance_id="i-1", now=110)
    reg.release("sows/a.md", instance_id="i-1", outcome="success", now=460, prs_opened=2)
    reg.try_acquire("dx/b.md", dispatch_id="d2", now=500, ttl_seconds=TTL)  # in flight
    return reg
...
def test_refresh_publishes_aggregated_gauges():
    ...
    assert sv("developer_history_runs_total", {"doc_type": "sow", "outcome": "success"}) == 1
    assert sv("developer_history_runs_total", {"doc_type": "dx", "outcome": "working"}) == 1
    assert sv("developer_history_prs_opened_total", {"doc_type": "sow", "outcome": "success"}) == 2
    assert sv("developer_history_compute_seconds_total", {"doc_type": "sow"}) == 360
